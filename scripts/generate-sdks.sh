...
#
# Usage:
#   ./scripts/generate-sdks.sh [--lang go|python|typescript]
#
# By default generates all three SDKs and downloads a verified copy of the spec.
#
# NOTE: These SDKs cover the REST API only. WebSocket types are generated by
# the TypeScript SDK from the published AsyncAPI spec.
...
SPEC_URL="https://developer.gemini.com/specs/openapi/rest.yaml"
SPEC_SHA256="79a0dc4061f3942dca8b30a589bbd406c781d2c6c19283d87cb21177afdcab5e"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      ...
      echo "Usage: $0 [--lang go|python|typescript]"
      exit 1
      ;;
  esac
done
...
download_spec() {
  ...
  if command -v sha256sum &>/dev/null; then
    actual_sha256=$(sha256sum "$SPEC_FILE" | awk '{print $1}')
  elif command -v shasum &>/dev/null; then
    actual_sha256=$(shasum -a 256 "$SPEC_FILE" | awk '{print $1}')
  else
    echo "Error: sha256sum or shasum is required to verify the OpenAPI spec."
    exit 1
  fi
  if [[ "$actual_sha256" != "$SPEC_SHA256" ]]; then
    echo "Error: OpenAPI spec hash mismatch."
    echo "Expected: $SPEC_SHA256"
...
echo "  - WebSocket types are generated separately in packages/sdk-typescript"
