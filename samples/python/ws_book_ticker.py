...
def stream_book_ticker(symbol: str):
    """Stream real-time best bid/ask for a given symbol."""
    ...
    def on_message(ws, data):
        ...
        print(f"[{time}] {msg['s']} bid {msg['b']} x {msg['B']} | ask {msg['a']} x {msg['A']}")
...
if __name__ == '__main__':
    ...
    if '--help' in sys.argv or '-h' in sys.argv:
        print('Usage: python3 ws_book_ticker.py [symbol]')
        print('Example: python3 ws_book_ticker.py ethusd')
        print('Default symbol: btcusd')
        sys.exit(0)

    # Get symbol from command line or use default
    symbol = sys.argv[1] if len(sys.argv) > 1 else 'btcusd'
    stream_book_ticker(symbol)
