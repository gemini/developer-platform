...
function streamPredictionPrices(symbol: string): void {
...
if (process.argv.includes('--help') || process.argv.includes('-h')) {
  console.log('Usage: npx ts-node src/pmPrices.ts [symbol]');
  console.log('Example: npx ts-node src/pmPrices.ts GEMI-PRES2028-VANCE');
  console.log('Default symbol: GEMI-PRES2028-VANCE');
  process.exit(0);
}

streamPredictionPrices(symbol);
