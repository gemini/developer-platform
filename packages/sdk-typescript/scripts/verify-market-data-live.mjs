...
const PREREQUISITE = "Prerequisite missing: @gemini-markets/sdk createClient marketData facade is not available in the packed package.";
export const OPERATIONS = [
  ...
  ["getNextFundingTimestamp", "public JSON"],
...
export function redact(value, secrets = []) {
  ...
  if (value && boundaryValueKind(value)=== "object") {
  ...
  if (boundaryValueKind(value)!== "string") return value;
  let redacted = value;
...
function operationResult(name, kind, status, message, evidence) {
  ...
  if (evidence) result.evidence = redact(evidence);
  if (status !== "passed") result.followUp = verificationFollowUp(name, message);
  return result;
}
...
function firstSymbol(symbols) {
  return Array.isArray(symbols) ? symbols.find((symbol) => boundaryValueKind(symbol)=== "string") : undefined;
}
...
export async function runVerification({ reportDir, env = process.env, loadSdk = () => import(process.argv.includes("--consumer") ? "@gemini-markets/sdk/server" : "../dist/server/index.js") } = {}) {
  ...
  const publicClient = await sdk.createClient({ env: env.GEMINI_MD_ENV ?? "production" });
  ...
  const call = async (client, name, args = []) => {
    if (boundaryValueKind(client.marketData[name])!== "function") throw new Error("method is not available on the marketData facade");
  ...
  const derivativeSymbol = env.GEMINI_MD_DERIVATIVE_SYMBOL ?? (Array.isArray(symbols)
    ? symbols.find((candidate) => boundaryValueKind(candidate)=== "string" && candidate.toUpperCase().includes("PERP"))
    : undefined);
  const publicCalls = [
    ...
    ["getNextFundingTimestamp", [{ symbol: derivativeSymbol }], Boolean(derivativeSymbol), "Required derivative symbol discovery returned no usable symbol."],
  ...
  if (!key || !secret) {
    ...
    const authenticatedClient = await sdk.createClient({ env: env.GEMINI_MD_ENV ?? "production", auth: new sdk.HmacAuth({ apiKey: key, apiSecret: secret }) });
    try {
      let network;
      try {
...
export function copyVerifierFiles(root, target) {
  cpSync(fileURLToPath(import.meta.url), join(target, "verify-market-data-live.mjs"));
  cpSync(join(root, "scripts", "runtime-value.mjs"), join(target, "runtime-value.mjs"));
}

export function runPackedVerifier({
  root = dirname(dirname(fileURLToPath(import.meta.url))),
  execFile = execFileSync,
  spawn = spawnSync,
  copyFiles = copyVerifierFiles,
} = {}) {
  ...
  try {
    const packed = JSON.parse(execFile("npm", ["pack", "--json", "--pack-destination", tempDir, "--cache", join(tempDir, ".npm")], { cwd: root, encoding: "utf8" }))[0];
    writeFileSync(join(tempDir, "package.json"), JSON.stringify({ type: "module", dependencies: { "@gemini-markets/sdk": `file:./${packed.filename}` } }));
    execFile("npm", ["install", "--ignore-scripts", "--no-package-lock", "--cache", join(tempDir, ".npm")], { cwd: tempDir, stdio: "inherit" });
    copyFiles(root, tempDir);
    const verificationProcess = spawn(
      process.execPath,
