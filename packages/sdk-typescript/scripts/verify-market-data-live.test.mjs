...
import { exitCodeFor, forwardedNodeExecArgs, OPERATIONS, redact, runVerification, verifyFileEvidence } from "./verify-market-data-live.mjs";
...
test("missing credentials block exactly authenticated operations", async () => {
  const reportDir = mkdtempSync(join(tmpdir(), "market-data-test-"));
  const methods = Object.fromEntries(["listSymbols", "getSymbolDetails", "getTicker", "getTickerV2", "getCurrentOrderBook", "listTrades", "listPrices", "listCandles", "listDerivativeCandles", "listFeePromos", "getFundingAmount", "getFundingAmountReportFile"].map((name) => [name, async () => name === "listSymbols" ? ["btcusd", "btcgusdperp"] : name === "getFundingAmountReportFile" ? { bytes: new Uint8Array([1]), contentType: "application/octet-stream", contentDisposition: "attachment; filename=report.xlsx" } : {}]));
  try {
    const results = await runVerification({ reportDir, env: {}, loadSdk: async () => ({ GeminiMarkets: class { constructor() { this.marketData = methods; } close() {} } }) });
...
test("uses generated Market Data arguments and runs FX when token lookup fails", async () => {
  ...
  const methods = Object.fromEntries(["listSymbols", "getSymbolDetails", "getTicker", "getTickerV2", "getCurrentOrderBook", "listTrades", "listPrices", "listCandles", "listDerivativeCandles", "listFeePromos", "getFundingAmount", "getFundingAmountReportFile", "getAssetsForNetwork", "getTokenNetworkV2", "getFXRate"].map((name) => [name, async (...args) => {
  ...
  try {
    ...
    await runVerification({ reportDir, env: { GEMINI_API_KEY: "key-secret", GEMINI_API_SECRET: "secret-value", GEMINI_MD_DERIVATIVE_SYMBOL: "btcgusdperp" }, loadSdk: async () => ({ GeminiMarkets: class { constructor() { this.marketData = methods; } close() {} }, HmacAuth: class {} }) });
    assert.deepEqual(Object.fromEntries(calls), {
      listSymbols: [], getSymbolDetails: [{ symbol: "btcusd" }], getTicker: [{ symbol: "btcusd" }], getTickerV2: [{ symbol: "btcusd" }], getCurrentOrderBook: [{ symbol: "btcusd" }, { limit_bids: 1, limit_asks: 1 }], listTrades: [{ symbol: "btcusd" }, { limit_trades: 1 }], listPrices: [], listCandles: [{ symbol: "btcusd", time_frame: "1m" }], listDerivativeCandles: [{ symbol: "btcgusdperp", time_frame: "1m" }], listFeePromos: [], getFundingAmount: [{ symbol: "btcgusdperp" }], getFundingAmountReportFile: [{ symbol: "btcgusdperp" }], getTokenNetworkV2: [{ token: "USDC" }], getFXRate: [{ symbol: "EURUSD", timestamp: 1770000000000 }],
    });
...
test("reports redact authentication headers and environment secrets", async () => {
  ...
  try {
    await runVerification({ reportDir, env: { GEMINI_API_KEY: "key-secret", GEMINI_API_SECRET: secret }, loadSdk: async () => ({ GeminiMarkets: class { constructor() { this.marketData = methods; } close() {} }, HmacAuth: class {} }) });
