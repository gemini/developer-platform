...
test("builds dynamic arguments for market-data and prediction-market calls", () => {
  ...
  assert.deepEqual(
    operationArgs({ module: "accountServices", methodName: "getAvailableBalances", method: "post", path: "/v1/balances" }, context),
...
test("routes prediction-market inventory entries to the predictions facade", async () => {
  const result = await runVerification({
    ...
    loadSdk: async () => ({
      GeminiMarkets: class {
        predictions = { async listEvents() { return []; } };
        close() {}
      },
    }),
    log: () => {},
...
test("passes the configured nonce mode to HmacAuth", async () => {
  ...
  await runVerification({
    ...
    loadSdk: async () => ({
      ...
      GeminiMarkets: class {
        marketData = { async listSymbols() { return ["btcusd"]; } };
        close() {}
      },
    }),
    log: () => {},
...
test("uses a bigint for timestamp_nanos fixtures", () => {
  const args = operationArgs(
    { module: "accountServices", methodName: "getTransactionHistory", method: "post", path: "/v1/transactions" },
    { env: {}, date: "2026-08-06", timestamp: 1_700_000_000_000 },
  );

  assert.equal(typeof args[0].timestamp_nanos, "bigint");
});
...
test("logs the server reason when an API request fails", async () => {
  const logs = [];
  const result = await runVerification({
    operations: [{ module: "accountServices", methodName: "getRoles", method: "post", path: "/v1/roles" }],
    env: {},
    loadSdk: async () => ({
      GeminiMarkets: class {
        accountServices = { async getRoles() { throw Object.assign(new Error("HTTP 400"), { reason: "MissingRole" }); } };
        close() {}
      },
    }),
