...
export function priceSymbols(response) {
  ...
  if (Array.isArray(source)) return source
    .map((entry) => typeof entry === "string" ? entry : entry?.symbol ?? entry?.pair)
    .filter(Boolean);
...
const QUERY_FIXTURES = {
  "marketData:getCurrentOrderBook": (context) => ({ limit_bids: 1, limit_asks: 1 }),
  "marketData:listTrades": () => ({ limit_trades: 1 }),
  "marketData:getFundingAmountReportFile": (context) => ({ symbol: context.derivativeSymbol, numRows: 1 }),
  ...
  "predictionMarkets:getLiquidityRewardsDailySummary": (context) => ({ dateFrom: "2026-05-01", dateTo: "2026-05-07" }),
...
const BODY_FIXTURES = {
  ...
  "accountServices:getAccountDetail": (context) => ({ account: account(context) }),
  "accountServices:getAvailableBalances": (context) => ({ account: account(context), showPendingBalances: false }),
  "accountServices:getGasFeeEstimation": (context) => context.env.GEMINI_MD_ADDRESS
    ? { address: context.env.GEMINI_MD_ADDRESS, amount: "1", account: account(context) }
    : MISSING_FIXTURE,
  "accountServices:getNotionalBalances": (context) => ({ account: account(context) }),
  "accountServices:getRoles": () => ({}),
  "accountServices:getTransactionHistory": (context) => ({ timestamp_nanos: BigInt(context.timestamp) * 1_000_000n, limit: 1 }),
  "accountServices:listAccountsInGroup": () => ({ limit_accounts: 1 }),
  "accountServices:listApprovedAddresses": (context) => ({ account: account(context) }),
  "accountServices:listCustodyFeeTransfers": (context) => ({ limit_transfers: 1, account: account(context) }),
  "accountServices:listDepositAddresses": (context) => ({ timestamp: String(context.timestamp), account: account(context) }),
  "accountServices:listPastTransfers": (context) => ({ currency: "eth", network: context.network, limit_transfers: 1, account: account(context) }),
  "accountServices:listPaymentMethods": (context) => ({ account: account(context) }),
  "accountServices:listStakingBalances": (context) => ({ account: account(context) }),
  "accountServices:listStakingEventHistory": (context) => ({ account: account(context), limit: 1, sortAsc: false }),
  "accountServices:listStakingRewards": (context) => ({ account: account(context), since: "2026-01-01T00:00:00.000Z" }),
...
const WRAPPED_INPUTS = new Set([
  "accountServices:getGasFeeEstimation",
  "accountServices:getNotionalBalances",
  "accountServices:listApprovedAddresses",
  "accountServices:listDepositAddresses",
  "perpetuals:getFundingPaymentReportFile",
  "perpetuals:getFundingPaymentReportJson",
  "perpetuals:listFundingPayments",
]);
...
export function operationArgs(operation, context) {
  ...
  if (path !== undefined) return query === undefined ? [path] : [path, query];
...
function eventTickerFrom(response) {
  ...
  const event = events.find((candidate) => candidate && typeof candidate === "object");
...
function instrumentSymbolFrom(response) {
  ...
  for (const event of events) {
    for (const market of [...(event?.markets ?? []), ...(event?.contracts ?? [])]) {
      const symbol = typeof market === "string" ? market : market?.symbol ?? market?.instrumentSymbol;
...
function comboSymbolFrom(response) {
  ...
  for (const combo of combos) {
    const symbol = typeof combo === "string" ? combo : combo?.instrumentSymbol ?? combo?.symbol;
    if (symbol) return symbol;
  }
  return undefined;
}
...
function updateContext(operation, response, context) {
  ...
  if (key === "marketData:listSymbols") {
    ...
    context.symbols = symbols.filter((symbol) => typeof symbol === "string");
...
export async function runVerification({
  ...
  let client;
  try {
    client = new sdk.GeminiMarkets({ env: env.GEMINI_SMOKE_ENV ?? "sandbox", auth });
    ...
    for (const operation of ordered) {
      ...
      try {
        ...
        if (!facade || typeof facade[operation.methodName] !== "function") {
          throw new Error("method is not available on GeminiMarkets facade");
        }
        ...
        results.set(key, { ...operationResult(operation, "passed"), ...(evidence ? { evidence } : {}) });
