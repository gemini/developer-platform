  ...
  const transport = new HttpTransport({
    ...
    fetchImpl: async (url, init) => {
      ...
      return {
        ...
        headers: jsonHeaders,
        async text() { return responses.shift() ?? "{}"; },
      };
    },
  });
...
void test("all eight T5d wrappers use authenticated schema methods and paths", async () => {
  ...
  const transport = new HttpTransport({
    ...
    fetchImpl: async (url, init) => {
      requests.push({ url, init });
      return { status: url.endsWith("/order") ? 201 : 200, headers: jsonHeaders, async text() { return "{}"; } };
    },
  });
  ...
  await rest.acceptPredictionMarketsTerms();
...
void test("T5d responses preserve partial batch results, bigint IDs, strings, and nulls", async () => {
  ...
  const rest = new PredictionMarketsRest(new HttpTransport({
    ...
    auth,
    fetchImpl: async (url) => ({
      status: url.endsWith("/order") ? 201 : 200,
      headers: jsonHeaders,
      async text() { return responses.shift()!; },
    }),
  }));
...
void test("T5d wrappers accept OAuth through the shared AuthStrategy seam", async () => {
  ...
  const rest = new PredictionMarketsRest(new HttpTransport({
    ...
    fetchImpl: async (url, init) => {
      requests.push({ url, init });
      return { status: 200, headers: jsonHeaders, async text() { return '{"hasAcceptedLatest":true}'; } };
    },
  }));
...
void test("T5d wrappers preserve mapped order-management failures", async () => {
  ...
  for (const [status, reason, Expected] of cases) {
    ...
    const rest = new PredictionMarketsRest(new HttpTransport({
      ...
      fetchImpl: async () => ({ status, async text() { return JSON.stringify({ reason }); } }),
    }));
...
void test("T5d wrappers preserve transport and malformed-response failures", async () => {
  ...
  const malformedRest = new PredictionMarketsRest(new HttpTransport({
    env: "sandbox", auth,
    fetchImpl: async () => ({ status: 200, headers: jsonHeaders, async text() { return "not json"; } }),
  }));
  await assert.rejects(malformedRest.acceptPredictionMarketsTerms(), SdkError);
});
...
void test("generated bodies cannot override the transport request path or nonce", async () => {
  ...
  const rest = new PredictionMarketsRest(new HttpTransport({
    ...
    fetchImpl: async () => {
      fetches++;
      return { status: 201, headers: jsonHeaders, async text() { return "{}"; } };
    },
  }));
  const order = {
    symbol: "GEMI-X", orderType: "limit" as const, side: "buy" as const,
  ...
  await assert.rejects(
    rest.placeOrder({ ...order, request: "/v1/other" } as Parameters<PredictionMarketsRest["placeOrder"]>[0]),
    EndpointMismatch,
  );
  await assert.rejects(
    rest.placeOrder({ ...order, nonce: 1 } as Parameters<PredictionMarketsRest["placeOrder"]>[0]),
    SdkError,
  );
...
void test("T5e responses preserve bigint IDs, exact money and volume strings, nulls, and omissions", async () => {
  ...
  const rest = new PredictionMarketsRest(new HttpTransport({
    env: "sandbox",
    auth,
    fetchImpl: async () => ({ status: 200, headers: jsonHeaders, async text() { return responses.shift()!; } }),
  }));
...
void test("T5f wrappers accept OAuth through the shared AuthStrategy seam", async () => {
  ...
  const rest = new PredictionMarketsRest(new HttpTransport({
    ...
    fetchImpl: async (url, init) => {
      requests.push({ url, init });
      return { status: 200, headers: jsonHeaders, async text() { return '{"daily_summaries":[]}'; } };
    },
  }));
