...
// applySnapshot(), and every other frame to ingest(). A snapshot has U == u; a diff need not.
...
test("applySnapshot rejects a non-snapshot frame (U != u) and recovers on a real one", () => {
  const book = new LiveOrderBook("btcusd");
  const errors: unknown[] = [];
  const updates: BookDelta[] = [];
  book.on("error", (e) => errors.push(e));
  ...
  book.applySnapshot(frame({ U: 10, u: 11, b: [["0.60", "5"]], a: [] })); // U != u — not a snapshot
  assert.equal(errors.length, 1);
  assert.ok(errors[0] instanceof SdkError);
  assert.equal(updates.length, 0);
  assert.equal(book.bestBid(), undefined, "never went live from a non-snapshot frame");
