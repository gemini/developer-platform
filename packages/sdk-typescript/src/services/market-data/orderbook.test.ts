...
import { normalizePrice, OrderBook } from "./orderbook.js";
...
function malformedLevels(value: BoundaryValue): string[][] {
  // SAFETY: These fixtures intentionally violate the wire-level string[][] contract so the order-book validator is exercised.
  return value as string[][];
}

function clearReadonlyMap(value: ReadonlyMap<string, string>): void {
  // SAFETY: This test intentionally crosses the readonly API boundary to verify the returned copy is independent.
  (value as Map<string, string>).clear();
}
...
test("spread and mid compute from best bid/ask", () => {
  ...
  assert.equal(book.spreadDecimal(), "1");
  assert.equal(book.midDecimal(), "100.5");
});

test("exact spread and midpoint preserve subcent prices", () => {
  const book = new OrderBook();
  book.applySnapshot({
    lastUpdateId: 1,
    bids: [["0.0020", "1"]],
  ...
  assert.equal(book.spreadDecimal(), "0.0005");
  assert.equal(book.midDecimal(), "0.00225");
  ...
  assert.equal(book.spreadDecimal(), "0.00000001");
  assert.equal(book.midDecimal(), "0.000000005");

  book.applySnapshot({
    lastUpdateId: 3,
    bids: [["0", "1"]],
    asks: [["0.00000000000000000001", "1"]],
  });
  assert.equal(book.midDecimal(), "0.000000000000000000005");
});

test("decimal sorting distinguishes subcent prices", () => {
  const book = new OrderBook();
  book.applySnapshot({
    lastUpdateId: 1,
    bids: [["0.0000099", "1"], ["0.00001", "2"]],
    asks: [],
  });

  assert.deepEqual(book.bestBid(), { price: "0.00001", qty: "2" });
});

test("fractional quantities retain wei-scale precision across updates", () => {
  const book = new OrderBook();
  book.applySnapshot({
    lastUpdateId: 1,
    bids: [["100", "0.000000000000000001"]],
    asks: [],
  });
  book.applyDiff(diff({ U: 1, u: 2, b: [["100", "0.000000000000000003"]] }));

  assert.equal(book.bids.get("100"), "0.000000000000000003");
});
...
test("read views are defensive and cannot mutate the book", () => {
  const book = seededBook();

  clearReadonlyMap(book.bids);
  assert.deepEqual(book.bestBid(), { price: "100.5", qty: "2" });

  const top = book.topN("bids", 1);
  const first = top[0];
  if (!first) throw new Error("expected a top bid");
  assert.throws(() => {
    first.price = "0";
  }, TypeError);
  assert.deepEqual(book.bestBid(), { price: "100.5", qty: "2" });
...
test("maintains state invariants through a deterministic diff sequence", () => {
  const book = new OrderBook();
  ...
  for (let step = 0; step < 100; step++) {
    const side = step % 2 === 0 ? bids : asks;
    const price = `${(step % 7) + 1}.${step % 3 === 0 ? "00" : step % 3}`;
    const quantity = step % 5 === 0 ? "0.00" : String(step + 1);
    const key = normalizePrice(price);
    if (quantity === "0.00") side.delete(key);
    else side.set(key, quantity);

    const id = BigInt(step + 1);
    const firstId = step === 0 ? id : id - 1n;
    book.applyDiff(diff({ U: firstId, u: id, b: step % 2 === 0 ? [[price, quantity]] : [], a: step % 2 === 1 ? [[price, quantity]] : [] }));

    assert.equal(book.lastUpdateId, id);
    assert.deepEqual([...book.bids.entries()], [...bids.entries()]);
    assert.deepEqual([...book.asks.entries()], [...asks.entries()]);
  }
});

test("bounds each side and preserves state when a level limit is exceeded", () => {
  const levels = Array.from({ length: 100_001 }, (_, index) => [String(index + 1), "1"]);
  const book = new OrderBook();
  book.applySnapshot({ lastUpdateId: 1, bids: [["1", "7"]], asks: [] });

  assert.throws(
    () => book.applySnapshot({ lastUpdateId: 2, bids: levels, asks: [] }),
    /100000-level limit/,
  );
  assert.equal(book.bids.get("1"), "7", "oversized snapshots do not clear the current book");
  assert.equal(book.lastUpdateId, 1n, "oversized snapshots do not advance the sequence");

  const additions = Array.from({ length: 100_000 }, (_, index) => [String(index + 2), "1"]);
  assert.throws(
    () => book.applyDiff(diff({ U: 1, u: 2, b: additions })),
    /100000-level limit/,
  );
  assert.equal(book.bids.size, 1, "oversized diffs do not partially mutate the side");
  assert.equal(book.lastUpdateId, 1n, "oversized diffs do not advance the sequence");
});

test("a replacement diff may add before removing when its final side stays within capacity", () => {
  const levels = Array.from({ length: 100_000 }, (_, index) => [String(index + 1), "1"]);
  const book = new OrderBook();
  book.applySnapshot({ lastUpdateId: 1, bids: levels, asks: [] });
  book.applyDiff(diff({ U: 1, u: 2, b: [["100001", "1"], ["1", "0"]] }));
  assert.equal(book.bids.size, 100_000);
  assert.equal(book.bids.has("1"), false);
  assert.equal(book.bids.get("100001"), "1");
});
...
test("a non-string level element is rejected before any mutation (atomic)", () => {
  ...
  const bad = diff({ U: 2, u: 2, b: malformedLevels([["100.5", "9"], [1, 2]]) });
...
test("a string masquerading as a level is rejected (not read as [char, char])", () => {
  ...
  const bad = diff({ U: 2, u: 2, b: malformedLevels(["12"]) });
  assert.throws(() => book.applyDiff(bad), SdkError);
  assert.equal(book.bids.get("100.5"), "2", "book untouched");
});

test("a malformed outer depth side is rejected before advancing or clearing the book", () => {
  const book = seededBook();
  const malformedDiff = diff({ U: 42, u: 43, b: malformedLevels({}) });

  assert.throws(() => book.applyDiff(malformedDiff), /malformed depth side/);
  assert.equal(book.bids.get("100.4"), "1", "malformed side does not silently retain stale state");
  assert.equal(book.lastUpdateId, 42n, "sequence is not advanced");

  assert.throws(
    () => book.applySnapshot({ lastUpdateId: 43, bids: malformedLevels({}), asks: [] }),
    /malformed depth side/,
  );
  assert.equal(book.lastUpdateId, 42n, "malformed snapshot does not clear or advance the book");
});
...
test("applySnapshot rejects a non-string level atomically (prior book left intact)", () => {
  ...
  assert.throws(
    () => book.applySnapshot({ lastUpdateId: 2, bids: malformedLevels([["99", "5"], [1, 2]]), asks: [] }),
    SdkError,
  );
