...
export function createMarketTools(client) {
    return [
        {
        ...
        },
        {
            ...
            handler: wrapHandler(({ symbol }) => market.getSymbolDetails(client, symbol)),
        },
        {
            name: 'gemini_get_ticker',
            description: 'Get ticker information for a symbol (v2)',
            inputSchema: z.object({ symbol: z.string().describe('Trading symbol e.g. btcusd') }),
            handler: wrapHandler(({ symbol }) => market.getTicker(client, symbol)),
        },
        {
            name: 'gemini_get_ticker_v1',
            description: 'Get ticker information for a symbol (v1)',
            inputSchema: z.object({ symbol: z.string().describe('Trading symbol e.g. btcusd') }),
            handler: wrapHandler(({ symbol }) => market.getTickerV1(client, symbol)),
        },
        {
            ...
            handler: wrapHandler(({ symbol, timeFrame }) => market.getCandles(client, symbol, timeFrame)),
        },
        {
            ...
            handler: wrapHandler(({ symbol, timeFrame }) => market.getDerivativeCandles(client, symbol, timeFrame)),
        },
        {
        ...
        },
        {
            ...
            handler: wrapHandler(({ symbol, limitTrades, since }) => market.getRecentTrades(client, symbol, limitTrades, since)),
        },
        {
            ...
            handler: wrapHandler(({ symbol, limitBids, limitAsks }) => market.getOrderBook(client, symbol, limitBids, limitAsks)),
        },
        {
            ...
            handler: wrapHandler(({ symbol, limitAuctions }) => market.getAuctionHistory(client, symbol, limitAuctions)),
        },
        {
        ...
        },
        {
            ...
            handler: wrapHandler(({ symbol }) => market.getCurrentFundingRate(client, symbol)),
        },
        {
            ...
            handler: wrapHandler(({ token }) => market.getNetworkCodes(client, token)),
        },
    ];
}
